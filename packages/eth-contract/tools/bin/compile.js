...
function downloadSolc(version) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let data = yield request("https://solc-bin.ethereum.org/bin/list.json");
            ...
            if (list) {
                ...
                if (file) {
                    ...
                    if (build) {
                        ...
                        let solcjs = yield request("https://solc-bin.ethereum.org/bin/" + filename);
                        if (!fs.existsSync(SolcjsPath))
...
function getSolc(version) {
    return __awaiter(this, void 0, void 0, function* () {
        let solcjsPath;
        if (version)
            solcjsPath = getCache(version);
        if (!solcjsPath) {
            solcjsPath = yield downloadSolc(version);
        }
        if (!solcjsPath) {
            return null;
        }
            ...
            if (overrides) {
                for (let s in overrides) {
                    if (overrides[s].version && overrides[s].version != version) {
                        solc = yield getSolc(overrides[s].version);
                    }
