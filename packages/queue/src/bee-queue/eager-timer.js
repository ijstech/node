...
//https://github.com/bee-queue/bee-queue/blob/1c2fb849708881408fe22c6527e3f62c33a58755/lib/eager-timer.js
const Emitter = require('events').EventEmitter;

/**
 * A timer that will eagerly replace an existing timer with a sooner one. Refuses to set a later
 * timer, or a timer beyond the given maximum delay.
 */
class EagerTimer extends Emitter {
  ...
  schedule(time) {
    ...
    if (time < 0 || time == null || isNaN(time)) {
      // Times earlier than 0 are used to start the default timer of the maximum delay.
      time = now + this._maxDelay;
    } else if (time <= now) {
      // If it's in the past, just trigger immediately, and reschedule to the maximum.
  ...
  _trigger() {
    ...
    /* istanbul ignore if */
    if (remaining > 0) {
      // It's possible (it caused bugs) to have the timeout trigger before the scheduled time.
