...
describe('DB Client', function() {
    this.timeout(20000);
    let PDMServer: HttpServer;
    let client: DBClient;
    let dbClient: DB.IClient;

    before(async function(){
        dbClient = DB.getClient(Config);
        client = new DBClient({
            url: 'http://localhost:9999/pdm'
        });
        PDMServer = new HttpServer({
            port:9999
        });        
        await PDMServer.start();      
        PDMServer.use(async (ctx: Koa.Context)=>{
            let data = (<any>ctx.request).body;
            let result = await dbClient.applyQueries(data);
            ctx.body = result;
        });
    });
    it('Query Customer where customerNumber in [112, 114]', async function(){
