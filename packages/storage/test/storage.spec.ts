...
describe('Storage', function () {
    ...
    it('Sync Dir', async function(){
        ...
        let result = await storage.syncDirTo(path, {s3: true})
        assert.strictEqual(result.cid, 'bafybeidyzdelnhfq2fus6wi2i6n2r7yqkf2fir6nlwqyzhsxwds2gwn7cm');
    });
    it('Sync Github', async function(){
        let storage = new Storage(Config);
        let result = await storage.syncGithubTo({org:'ijstech',repo:'openswap-scbook',commit:'f1abac737421db53e507be21dafc6710a73c8c6f'}, {s3: true, ipfs: true});
    ...
    it('get File Content', async function(){
        let storage = new Storage(Config);
        let result = await storage.getFileContent('bafybeidyzdelnhfq2fus6wi2i6n2r7yqkf2fir6nlwqyzhsxwds2gwn7cm', 'folder1/file2.txt')
