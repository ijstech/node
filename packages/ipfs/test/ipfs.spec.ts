import { ICidInfo, hashDir, hashItems, hashContent, hashFile, hashFile1 } from '../src/index';
...
describe('IPFS', function () {
  ...
  // > 1MB
  // it('hash items by old lib >1MB', async () => {
  ...
  // < 1MB
  // it('hash items by old lib <1MB', async () => {
  //   let { cid } = await hashFile(Path.resolve(__dirname, './1048575.bin'), 0);
  //   assert.strictEqual(cid, 'Qmci7PX2Dj9SK3B5ZDsQ4FXgddMh9qQyf1nitiWFYCSGYj');
  // });
  // it('hash items by new lib <1MB', async () => {
  //   let stream = fs.createReadStream(Path.resolve(__dirname, './1048575.bin'));
  //   let c = await Hash.of(stream);
  //   assert.strictEqual(c, 'Qmci7PX2Dj9SK3B5ZDsQ4FXgddMh9qQyf1nitiWFYCSGYj');
  // });
